...
class RedBookPlatform:
    ...
    def parse_video(self):
        ...
        result = {}
        if note_data['note']['type'] == "normal":  # 图片
            image_resource = [img['urlDefault'] for img in note_data['note']['imageList']]
            result = {
                'type': 2,
                'title': note_data['note']['title'],
                'desc':  note_data['note']['desc'],
                'cover': note_data['note']['imageList'][0]['urlDefault'],
                'resourcePath': image_resource
            }

        elif note_data['note']['type'] == "video":  # 视频
            video_url = ""
            if len(note_data['note']['video']['media']['stream']['h264']) > 0:
                video_url = note_data['note']['video']['media']['stream']['h264'][0]['masterUrl']
            result = {
                'type': 1,
                'title': note_data['note']['title'],
                'desc':  note_data['note']['desc'],
